...
*T**ypes of repetitive research ordered by reproduction and replication and respective closeness to the original study.*
...
![Reproduction decision tree](images/WBn_Image_3.png)

## Close replication before conceptual replication

If the goal is to increase the generalizability of a specific finding, we also suggest starting with replications that adhere as close as possible to the original study (e.g., close replications) and only later conduct conceptual replications. Based on [-@HueffmeierEtAl2016], we propose the typology and order of replication attempts in Figure 3. Importantly, replications at any stage should not compromise any aspects of an original study, but rather (at the latest from the third study stage [constructive replications] onwards) try to improve one or more aspects of the original study, such as “[…] more valid measures, more critical control variables, a more realistic task, a more representative sample, or a design that allows for stronger conclusions regarding causality”, see Köhler and Cortina [-@KohlerCortina2021, p. 494]). Köhler and Cortina term such replications “constructive replications” and caution against the conduct of “quasi-random” replications that vary features without clear rationale. 

Finally, there may be cases where the sequence of replications is not necessary, or where the context of the replication team requires a focus on generalisability to a specific context (see section [The Role of Differences for the Interpretation of Findings](?tab=t.0#heading=h.2nym03an83xp)).


**Figure**** 3**** \
***Sequence of replications from exact replications to conceptual replications under field conditions*

![Pathway from exact to conceptual replications](images/6sH_Image_4.png)
